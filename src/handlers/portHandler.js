...
export function handlePortConnect(port) {
  ...
  port.onMessage.addListener(function (msg) {
    console.log(
      '[Aiki Debug] Port message received:',
      msg,
      'parsed as:',
      msg.split(': ')[1],
    );
    switch (msg.split(': ')[1]) {
      ...
      case 'origin': {
        ...
        console.log('[Aiki Debug] Background received origin message:', {
          msg,
          action,
          segments,
        });
        (async () => {
          ...
          if (tabId === undefined) {
            try {
              ...
              console.warn(
                '[Background] Failed to get active tab on origin:origin:',
                _,
              );
            }
          }
          console.log('[Aiki Debug] Calling redirection.gotoOrigin with:', {
            action,
            tabId,
            type: 'popup',
          });
